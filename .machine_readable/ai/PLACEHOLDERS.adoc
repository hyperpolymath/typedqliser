...
sed -i 's/{{project}}/my_project/g' $(grep -rl '{{project}}' .)
sed -i 's/typedqliser/my-project/g' $(grep -rl 'typedqliser' .)
...
| `typedqliser` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
